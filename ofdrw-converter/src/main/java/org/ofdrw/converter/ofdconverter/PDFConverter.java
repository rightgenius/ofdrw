 ...
 * PDF转换为OFD转换器
 * <p>
 * 当前为桩实现：OpenPDF / iText 7 均不提供 PDF 页面栅格化能力（PDFBox 唯一），
 * 而 PDFBox 已被本次替换彻底移除。如需 PDF→OFD 功能，请使用基于 PDFBox 的
 * 历史分支，或者等待后续接入独立的 PDF 栅格化方案。
 *
 * @author 权观宇
...
public class PDFConverter implements DocConverter {
    ...
    public void convert(Path filepath, int... indexes) throws GeneralConvertException {
        throw new GeneralConvertException(
                "PDF→OFD 转换当前未实现：本次替换彻底移除了 PDFBox，而 OpenPDF / iText 7 " +
                        "均不提供 PDF 页面栅格化能力。如需该功能，请基于历史分支运行或接入独立 PDF 栅格化方案。");
    }
